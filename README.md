...
- 支持同尺寸多 SKU 分配：统一输入一组纸箱长宽高，SKU 数量支持 2 到 5 个，按 A/B/C 顺序管理颜色、目标装载量和装载顺序。
- 支持两种多 SKU 策略：不同卸货地点按 A/B/C 依次装载，同一卸货地点尽量保持完整装载面。
...
| 纸箱尺寸 | 集装箱 | 实际装载量 | 说明 |
| --- | --- | ---: | --- |
| `488 x 380 x 291 mm` | `40HQ` | `1,340` 箱 | 前部角件干涉区域少 1 箱，后部满放 |
...
- 多 SKU 当前定义为“同规格纸箱按客户 / 颜色 / 卸货顺序分配”。页面只开放一组公共纸箱尺寸，算法层也会拒绝异尺寸 SKU；如果要支持真实异尺寸多 SKU，需要重做装载算法。
- 单 SKU 算法只考虑纸箱底面长 / 宽两种朝向，不考虑高向旋转、承重、重心、层间稳定、装卸通道、纸箱压缩、公差和人工操作安全距离。
