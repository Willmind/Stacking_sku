...
> **产品可读**：产品可重点阅读“当前能力”“批量导入 Excel”“验证基准”和“当前实现边界”；技术栈、命令和目录结构主要面向研发与交付。
...
- 单 SKU 和多 SKU 默认启用 `长 x 宽 x 高` 与 `宽 x 长 x 高` 两种水平朝向；可按需开启侧放，在六种长宽高轴向排列中选择。
...
- 支持查看和导出每个纸箱的总ID、原始尺寸、机器人 `XYZABC`、所属层、所属排和排内ID。
- 支持浅色、深色和跟随系统三种全局外观模式。
...
`npm run test:e2e` 会先构建 `dist/`，再启动独立的 Vite Preview 服务，并用浏览器验证页面输入、计算结果、2D 画布、3D 画布和柜门标识。这样可以同时发现仅在生产构建中出现的资源加载和分包问题。

当前端到端测试还覆盖批量导入、结果文件下载、纸箱坐标导出、导入 loading 进度条、2D/3D 放大弹窗、主题切换、颜色选择、下拉选择、桌面和移动视口布局。
...
- 单 SKU 和多 SKU 可在六种长宽高轴向排列中按勾选项尝试，但不区分同一轴向排列下的标签面、抓取面或倒置语义，也不考虑承重、重心、层间稳定、装卸通道、纸箱压缩、公差和人工操作安全距离。
- 页面目前只开放预设柜型选择，自定义柜体长宽高输入仍在代码中保留过痕迹，但未作为当前功能暴露。
- 3D 渲染为前端实时展示，超大装载量场景会优先保证交互性能，不应把 3D 画面当作唯一交付依据；关键数量仍以算法结果和 2D 排布为准。
- 核心装载算法已纳入严格 TypeScript 检查；新增或调整算法内部结构时需要保持 `npm run build` 和基准测试通过。
