...
每个新任务或新窗口都按以下顺序恢复项目上下文：

1. 阅读本文件。
2. 阅读 `docs/README.md`，按其中的任务路由选择业务文档。
3. 修改坐标、排序、装载量或算法前，必须阅读对应的现行业务规则和测试。
4. 使用 `git log`、`git show` 追溯最近相关提交；不要依赖其他聊天窗口中的隐含记忆。
5. 如果历史规格、计划与现行代码或业务文档冲突，以 `docs/README.md` 定义的信息优先级为准，并明确指出冲突。

当前关键资料：

- 机器人坐标、纸箱尺寸与 `XYZABC`：`docs/business/coordinate-system.md`
- 总ID、所属层、所属排与排内ID：`docs/packing-terminology.md`
- 装柜基准案例：`docs/business/benchmark-cases.md`
- 项目数据流和模块边界：`docs/architecture.md`
- 表单与批量输入：`docs/project-input-requirements.md`
- 已接受的重要决策：`docs/decisions/`

## 项目知识维护

- 业务规则改变时，同一需求中同步修改代码、自动化测试和对应的现行业务文档。
- 重要且长期有效的选择写入 `docs/decisions/`，记录背景、决定和影响。
- `docs/superpowers/specs/` 与 `docs/superpowers/plans/` 是需求过程和历史记录，不默认代表当前业务事实。
- 不在多个现行文档中复制同一套详细规则；指定一个事实来源，其他文档通过链接引用。
- 发现文档与实现不一致时，不得静默选择其一；先结合测试和当前用户要求确认，再修正文档或代码。
- 完成任务前检查文档链接、关键数值和示例是否仍与测试一致。
